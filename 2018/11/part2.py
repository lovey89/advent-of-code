...
def create_summed_area_table(power_grid):
    summed_area_table = [[0 for y in range(grid_size)] for x in range(grid_size)]
    for x in range(grid_size):
        for y in range(grid_size):
            ...
            summed_area_table[x][y] = left + above - diag + power_grid[x][y]
...
if __name__ == '__main__':
    ...
    print(max_power, max_power_x + 1, max_power_y + 1, max_power_s) # The grid is off by one hence we need to add 1
