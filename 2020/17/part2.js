...
for (let y = 0; y < input.length; y++) {
  for (let x = 0; x < input[0].length; x++) {
    initialCube[generations][generations][generations + y][generations + x] = input[y][x];
  }
}
...
function nextGen(cube) {
  ...
  for (let t = 0; t < tMax; t++) {
    for (let z = 0; z < zMax; z++) {
      for (let y = 0; y < yMax; y++) {
        for (let x = 0; x < xMax; x++) {
          let count = neighbors(cube, x, y, z, t);
          let char = cube[t][z][y][x];
          if (char == '.' && count == 3) {
            nextGenCube[t][z][y][x] = '#';
          } else if (char == '#' && !(count == 3 || count == 2)) {
            nextGenCube[t][z][y][x] = '.';
          } else {
            nextGenCube[t][z][y][x] = char;
          }
        }
      }
    }
...
function neighbors(cube, xc, yc, zc, tc) {
  let count = 0;
  for (let t = Math.max(0, tc - 1); t < Math.min(tMax, tc + 2); t++) {
